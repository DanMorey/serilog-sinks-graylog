...
namespace Serilog.Sinks.Graylog.Core.MessageBuilders
{
    ...
    public class ExceptionMessageBuilder : GelfMessageBuilder
    {
        ...
        public override JsonObject Build(LogEvent logEvent)
        {
            ...
            {
                if (logEvent.Exception.GetType().FullName == "Serilog.Formatting.Compact.Reader.TextException")
                {
                    // If the exception has been serialised and deserialised using the Serilog.Compact.Reader package then all you can do with them is call ToString().
                    string exceptionString = logEvent.Exception.ToString();
                    logEvent.AddOrUpdateProperty(new LogEventProperty("ExceptionString", new ScalarValue(exceptionString)));
                }
                else
                {
